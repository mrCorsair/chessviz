...
int check1(){
	...
	case 'p':
		...
		if((desk[x2][y2]==(char)219||desk[x2][y2]==' ') && x1==6 && y11==y2 && x1-x2>0 && x1-x2<3) return 1;//ходы по пустым клеткам с начальной позиции
...
int check2(){
	...
	case 'P':
		...
		if((desk[x2][y2]==(char)219||desk[x2][y2]==' ') && x1==1 && y11==y2 && x2-x1>0 && x2-x1<3) return 1;//по пустым клеткам с начальной позиции
